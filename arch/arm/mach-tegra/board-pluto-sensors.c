...
#define NTC_47K_TGAIN   0xE682
#define NTC_47K_TOFF    0x26AF
#define MAX77665_CHARGER_INT	TEGRA_GPIO_PJ2
#define MAX17042_ALERT_GPIO	    TEGRA_GPIO_PJ0

#ifdef CONFIG_SWITCH
#include <linux/switch.h>
static struct switch_dev cam_switch;

static ssize_t cam_switch_print_state(struct switch_dev *sdev, char *buf)
{
	if (!sdev->state)
...
static int cam_switch_init(void)
{
	int ret;

	cam_switch.name = "camera";
	cam_switch.state = 0;
	cam_switch.print_state = cam_switch_print_state;
	ret = switch_dev_register(&cam_switch);
	if (ret < 0)
		pr_err("failed to register switch driver\n");
	return ret;
}
#endif
/*+++ ZTE: added by liliguo for sensors init 20121217 +++*/
#ifdef CONFIG_ZTE_SENSORS
extern int zte_sensors_init(void);
#endif
 /*--- ZTE: added by liliguo for sensors init 20121217 ---*/

/* +++ZTE: added by liliguo for mhl init 20130118 +++ */
#ifdef CONFIG_SII8240_MHL_TX
extern int __init zte_mhl_init(void);
#endif
/* ---ZTE: added by liliguo for mhl init 20130118--- */
#if 0
static struct nvc_gpio_pdata imx091_gpio_pdata[] = {
	...
	{IMX091_GPIO_GP2, CAM_GPIO2, true, false},
};
#endif

static struct board_info board_info;
//ZTE:delete the origin data and use the data from maxim by caixiaoguang 20130218
#if 0
...
static struct max17042_config_data conf_data = {
	.valrt_thresh = 0xff00,
	.talrt_thresh = 0x37FF,//max 55,min -1
	.soc_alrt_thresh = 0xff00,
	.shdntimer = 0xe000,
	.design_cap = 0x11F8,
	.at_rate = 0x0000,
	...
	.cell_char_tbl = {
		/* Data to be written from 0x80h */
		0x89F0,0xB700,0xB870,0xB9A0,0xBB60,0xBC60,0xBD40,0xBE70,
        0xBFA0,0xC090,0xC330,0xC5E0,0xC8C0,0xCBA0,0xD140,0xD6F0,
		/* Data to be written from 0x90h */
		0x00B0,0x1040,0x1340,0x0B10,0x1510,0x2270,0x16F0,0x10F0,
        0x1AD0,0x0910,0x08E0,0x08E0,0x08E0,0x06F0,0x06F0,0x06F0,
		/* Data to be written from 0xA0h */
...
/*ZTE:add by caixiaoguang 20130304 for charger status ++*/
extern int palmas_vbus_status(void);
extern int max77665_get_vbus_ovp_status(void);//ZTE:ZTE:add by caixiaoguang for charge
extern int max17042_get_otp_status(void);
extern int smps10_is_enabled;
static int charger_is_online(void)
{
	int charger_present = -1;
    if(1 == smps10_is_enabled)
        {
            charger_present = 0;
            printk("[%s]CXG get OTG is open,so vbus is not charger\n",__func__);
        }
    else
        {
            charger_present = palmas_vbus_status();
        }
...
static unsigned max77665_f_estates[] = { 3500, 889, 560, 456, 0 };
...
static struct max77665_charger_plat_data max77665_charger = {
	.fast_chg_cc = 999, /* fast charger current*/
	.term_volt = 4350, /* charger termination voltage */
	.curr_lim = 1000, /* input current limit */
...
static struct nct1008_platform_data pluto_nct1008_pdata = {
	...
	.offset = 0,
	.shutdown_ext_limit = 102, /* C */
	.shutdown_local_limit = 102, /* C */
...
static int pluto_get_extra_regulators(void)
{
	...
	if (!pluto_i2cvdd) {
		pluto_i2cvdd = regulator_get(NULL, "avdd_cam1");
...
static int pluto_focuser_power_on(struct bu64291_power_rail *pw)
{
	int err;
...
static int pluto_focuser_power_off(struct bu64291_power_rail *pw)
{       
	regulator_disable(pluto_vcmvdd);
//	regulator_put(pluto_vcmvdd);
...
static struct nvc_imager_cap imx091_cap = {
	...
	.focuser_guid		= FOCUSER_bu64291_GUID,
...
#if 0
/* yangchunni add start */
static int pluto_ov9740_power_on(void)
{
	int err;
	...
	if (pluto_get_extra_regulators())
		goto pluto_ov9740_poweron_fail;
...
pluto_ov9740_poweron_fail:
	pr_err("%s failed.\n", __func__);
	return -ENODEV;
}

static int pluto_ov9740_power_off(void)
{
	if (unlikely(WARN_ON(!pluto_i2cvdd || !pluto_vcmvdd)))
		return -EFAULT;
...
struct ov9740_platform_data pluto_ov9740_pdata = {
	.power_on = pluto_ov9740_power_on,
	.power_off = pluto_ov9740_power_off,
};
#endif 
static int pluto_ov12830_power_on(struct ov12830_power_rail *pw)          
{                                                                         
	int err;                                                                

     printk("zhangduo test %s +++ \n",__func__);
      if (pluto_get_extra_regulators())                                       
		goto pluto_ov12830_poweron_fail;
    gpio_set_value(CAM2_POWER_DWN_GPIO,0);
    gpio_set_value(CAM1_POWER_DWN_GPIO, 0);                                 
    gpio_set_value(CAM_RSTN, 0); 
      
	
    gpio_set_value(CAM_GPIO1, 1);  /*  DOVDD 1.8V*/
    gpio_set_value(CAM1_POWER_DWN_GPIO, 1);
    mdelay(2);

    gpio_set_value(CAM_GPIO2, 1); /* DVDD 1.2V*/
    mdelay(2);
	 
    err = regulator_enable(pluto_i2cvdd);  /* AVDD 2.8V*/
    if (unlikely(err))		
        goto ov12830_i2c_fail;
        mdelay(2);
/* deleted by zhangduo 20130409
    err = regulator_enable(pluto_vcmvdd);                                   
    if (unlikely(err))                                                      
        goto ov12830_vcmvdd_fail;  
	mdelay(2);
*/	
	gpio_set_value(CAM_RSTN, 1); 
...
/*
ov12830_vcmvdd_fail:
	printk("\n pluto_ov12830_power_on__ov12830_vcmvdd_fail \n ");
*/      
ov12830_i2c_fail:
	
	tegra_pinmux_config_table(&mclk_disable, 1); 
pluto_ov12830_poweron_fail:                                               
	pr_err("%s FAILED\n", __func__);                                        
	return -ENODEV;                                                         
} 
static int pluto_ov12830_power_off(struct ov12830_power_rail *pw)         
{      
...
static struct nvc_gpio_pdata ov12830_gpio_pdata[] = {                     
	{ OV12830_GPIO_TYPE_PWRDN, CAM1_POWER_DWN_GPIO, true, 0, },             
	{ OV12830_GPIO_TYPE_SHTDN, CAM_RSTN, true, 0, },   
	{OV12830_GPIO_GP1, CAM_GPIO1, true, false},
	{OV12830_GPIO_GP2, CAM_GPIO2, true, false},
};

static unsigned ov12830_estates[] = { 471, 332, 0 };
static struct ov12830_platform_data pluto_ov12830_pdata = {               
	.num		= 0,                                                            
	.dev_name	= "camera",                                                   
	.gpio_count	= ARRAY_SIZE(ov12830_gpio_pdata),                           
	.gpio		= ov12830_gpio_pdata,                                           
	.power_on	= pluto_ov12830_power_on,                                     
	.power_off	= pluto_ov12830_power_off,
	.edpc_config    = {
		.states = ov12830_estates,
		.num_states = ARRAY_SIZE(ov12830_estates),
		.e0_index = ARRAY_SIZE(ov12830_estates) - 1,
		.priority = EDP_MAX_PRIO + 1,
	},
...
static struct bu64291_platform_data pluto_bu64291_pdata = {
	.cfg		= 0,
	.num		= 0,
	.sync		= 0,
	.dev_name	= "focuser",
...
static struct i2c_board_info pluto_i2c_board_info_e1625[] = {
	{	I2C_BOARD_INFO("ov12830", 0x36),
		.platform_data = &pluto_ov12830_pdata, 	
	},
	{	I2C_BOARD_INFO("bu64291", 0x18>>1),
		.platform_data = &pluto_bu64291_pdata, 	
	},
      {
	    I2C_BOARD_INFO("imx132", 0x13), //the real add is 0x36
...
static int pluto_camera_init(void)
{
	...
	tegra_pinmux_config_table(&mclk_disable, 1);
	tegra_pinmux_config_table(&pbb0_disable, 1);
      ret = gpio_request(CAM2_POWER_DWN_GPIO,"cam2_pwdn");
		if (ret < 0) 
		    pr_err("gpio_request failed for gpio cam2_pwdn\n");
	ret = gpio_direction_output(CAM2_POWER_DWN_GPIO,0);
		if (ret < 0) {
		    pr_err("gpio_direction_output failed for gpio cam2_pwdn \n");
		    gpio_free(CAM2_POWER_DWN_GPIO);
		    }
		
	 ret = gpio_request(CAM_AF_PWDN,"cam2_reset");
		if (ret < 0) 
		    pr_err("gpio_request failed for gpio cam2_reset\n");
	ret = gpio_direction_output(CAM_AF_PWDN,0);
		if (ret < 0) {
		    pr_err("gpio_direction_output failed for gpio cam2_reset \n");
		    gpio_free(CAM_AF_PWDN);
		    }
...
/*+++ ZTE: modified by liliguo for removing the platform sensor 20121217 +++*/
#if 0
...
#endif
/*--- ZTE: modified by liliguo for removing the platform sensor 20121217 ---*/
/*ZTE:add by caixiaoguang 20130304 for led ++*/
struct notifications_leds_pdata {
	void (*red_control)(int on_off);
	void (*green_control)(int on_off);
	void (*blue_control)(int on_off);
};
extern void palmas_redlight_ctrl(int on_off);
extern void palmas_greenlight_ctrl(int on_off);

static void led_red_control(int on_off)
{
	palmas_redlight_ctrl(on_off);
}

static void led_green_control(int on_off)
{
	palmas_greenlight_ctrl(on_off);
}

struct notifications_leds_pdata leds_pdata = {
	.red_control = led_red_control,
	.green_control = led_green_control,
};

static struct platform_device leds_platform_device = {
...
static struct throttle_table skin_throttle_table[] = {
	...
	{ { 1200000, NO_CAP, NO_CAP, NO_CAP, NO_CAP } },
//	{ { 1200000, NO_CAP, NO_CAP, NO_CAP, NO_CAP } },
//	{ { 1100000, NO_CAP, NO_CAP, NO_CAP, NO_CAP } },
//	{ { 1100000, NO_CAP, NO_CAP, NO_CAP, NO_CAP } },
//	{ { 1000000, NO_CAP, NO_CAP, NO_CAP, NO_CAP } },
//	{ { 1000000, NO_CAP, NO_CAP, NO_CAP, NO_CAP } },
	{ {  900000, NO_CAP, NO_CAP, NO_CAP, NO_CAP } },
	{ {  900000, NO_CAP, NO_CAP, NO_CAP, NO_CAP } },
//	{ {  900000, NO_CAP, NO_CAP, NO_CAP, NO_CAP } },
//	{ {  900000, NO_CAP, NO_CAP, NO_CAP, NO_CAP } },
	{ {  800000, NO_CAP, NO_CAP, NO_CAP, NO_CAP } },
//	{ {  800000, NO_CAP, NO_CAP, NO_CAP, NO_CAP } },
//	{ {  800000, NO_CAP, NO_CAP, NO_CAP, 624000 } },
//	{ {  800000, NO_CAP, NO_CAP, NO_CAP, 624000 } },
	{ {  800000, NO_CAP, NO_CAP, NO_CAP, 624000 } },
	{ {  750000, NO_CAP, NO_CAP, NO_CAP, 624000 } },
	{ {  750000, NO_CAP, NO_CAP, NO_CAP, 624000 } },
//	{ {  750000, NO_CAP, NO_CAP, NO_CAP, 624000 } },
//	{ {  750000, NO_CAP, NO_CAP, NO_CAP, 624000 } },
	{ {  700000, 564000, NO_CAP, NO_CAP, 624000 } },
	{ {  700000, 564000, 456000, NO_CAP, 624000 } },
//	{ {  700000, 564000, 456000, NO_CAP, 624000 } },
//	{ {  700000, 564000, 456000, NO_CAP, 624000 } },
	{ {  650000, 564000, 408000, NO_CAP, 624000 } },
	{ {  650000, 564000, 408000, 208000, 624000 } },
//	{ {  650000, 564000, 408000, 208000, 624000 } },
//	{ {  650000, 528000, 408000, 208000, 624000 } },
	{ {  600000, 528000, 324000, 204000, 624000 } },
	{ {  600000, 528000, 324000, 204000, 624000 } },
//	{ {  600000, 528000, 324000, 204000, 624000 } },
	{ {  550000, 528000, 324000, 136000, 624000 } },
//	{ {  550000, 528000, 324000, 136000, 408000 } },
	{ {  550000, 492000, 252000, 136000, 408000 } },
	{ {  500000, 492000, 252000, 102000, 408000 } },
	{ {  450000, 492000, 252000, 102000, 408000 } },
//       { {  400000, 324000, 204000, 102000, 408000 } },
};
...
int __init pluto_sensors_init(void)
{
	...
	//printk("%s: ++\n", __func__);
	pluto_camera_init();
    platform_device_register(&leds_platform_device);
/*+++ ZTE: added by liliguo for sensors init 20121217 +++*/
#ifdef CONFIG_ZTE_SENSORS
	zte_sensors_init();
#endif
...
/* +++ZTE: added by liliguo for mhl init 20130118+++ */
	...
	err = zte_mhl_init();
	if (err < 0) {
		pr_err("%s: zte_mhl_init failed %d, line: %d\n", __func__, err, __LINE__);
		return err;
	}
#endif
/* ---ZTE: added by liliguo for mhl init 20130118--- */
    ...
    max17042_device[0].irq = gpio_to_irq(MAX17042_ALERT_GPIO);
