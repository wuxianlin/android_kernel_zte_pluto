...
extern void ztepluto_keys_init(void);
...
#if defined(CONFIG_LEDS_PWM)
static struct led_pwm keybl_pwm_leds[] = {
	{
		.name		= "keybl",
		.pwm_id 	= 0,
		.max_brightness = 255,
...
static struct led_pwm_platform_data keybl_pwm_data = {
	.num_leds	= ARRAY_SIZE(keybl_pwm_leds),
	.leds		= keybl_pwm_leds,
};

static struct platform_device keybl_leds_pwm = {
	.name	= "leds_pwm",
	.id = -1,
	.dev	= {
		.platform_data = &keybl_pwm_data,
	},
};
#endif

#if 0 //ZTE: delete platform audio driver by wangtao
...
#ifdef CONFIG_LC171X_MODEM_EDP
/**
 * |-----------------------------------------------------------------|
 * | estate | Max PC (mW) | Descirption                              |
 * |-----------------------------------------------------------------|
 * | E0     | 1200        | 3G enabled                               |
 * | E1     | 1200        | Voice call actived                       |
 * | E2     | 400         | Modem boot                               |
 * | E3     | 40          | Modem suspend                            |
 * | E4     | 0           | Modem power off                          |
 * |-----------------------------------------------------------------|
 */
static unsigned int lc171x_modem_edp_states[] = {400, 400, 400, 400, 0};

static struct edp_client lc171x_modem_edp_client = {
        .name = "lc171x_modem",
        .states = lc171x_modem_edp_states,
        .num_states = ARRAY_SIZE(lc171x_modem_edp_states),
        .e0_index = 0,
        .priority = EDP_MAX_PRIO,
};

static struct oem_modem_edp_data lc171x_modem_edp_data = {
        .edp_mgr_name = "battery",
        .edp_client = &lc171x_modem_edp_client,
};
#endif

/*++ cuijian add for modem driver 20121213 ++*/
...
static void __init pluto_uart_init(void)
{
	...
	pluto_loopback_uart_pdata.is_modem = 0;
/*++ cuijian add for modem driver 20121213 ++*/
	modem_uart_pdata = pluto_uart_pdata;
	modem_uart_pdata.wake_peer = wake_modem;
	modem_uart_pdata.sleep_ctrl = modem_sleep_control;
	modem_uart_pdata.modem_edp_pdata = &lc171x_modem_edp_data;
	modem_uart_pdata.is_modem = 1;	
...
static struct tegra_usb_platform_data tegra_ehci1_utmi_pdata = {
	...
	.has_hostpc = true,
	.id_det_type = TEGRA_USB_ID,
#ifdef CONFIG_TEGRA_SUPPORT_PMU_VBUS
	.support_pmu_vbus = true,
#endif	
	.unaligned_dma_buf_supported = false,
	.phy_intf = TEGRA_USB_PHY_INTF_UTMI,
...
static struct i2c_board_info synaptics_i2c_board_info[] = {
	{
		...
		//.irq = gpio_to_irq(TEGRA_GPIO_PK2),		
	},
};
#endif
...
static int __init pluto_touch_init(void)
{
	...
	touch_init_raydium(TOUCH_GPIO_IRQ_RAYDIUM_SPI,
      ...
      gpio_request(TEGRA_GPIO_PK4, "touch-reset");
...
#define GPS_ON_OFF_GPIO		3
#define GPS_RESET_GPIO			164
#define GPS_POWER_GPIO			68

static void gps_sirf_gpio_init(void)
{
	int ret = -1;
	printk(KERN_DEBUG "gps_sirf_gpio_init\n");

	ret = gpio_request(GPS_POWER_GPIO, "gps_power");
	if (0 != ret)
	{
		printk(KERN_ERR "GPS_POWER_GPIO fail\n"); 
		goto fail_err;
	}
	
	ret = gpio_request(GPS_RESET_GPIO, "gps_reset");
	if (0 != ret) {
		printk(KERN_ERR "GPS_RESET_GPIO fail\n"); 
	...
	if (0 != ret) 
	{
		printk(KERN_ERR "GPS_ON_OFF_GPIO fail\n");
		gpio_free(GPS_POWER_GPIO);
	...
	gpio_direction_output(GPS_ON_OFF_GPIO, 0);
       return;
fail_err:
	printk(KERN_ERR "gps_sirf_gpio_init fail\n"); 
       return;
...
#define GPSMSG_LENGTH    	32
static char sirf_status[GPSMSG_LENGTH+1] = "off";
		...
		int count, int *eof, void *data)
{
	...
	{
           printk(KERN_ERR "invalid input argument\n");
	}
	return len;
}
...
{	
	...
	char buffer[GPSMSG_LENGTH+1];
	char messages[GPSMSG_LENGTH+1];

	memset(messages,0,sizeof(messages));
	memset(buffer,0,sizeof(buffer));

       lenknl = len;
	if (lenknl > GPSMSG_LENGTH)
	{
		lenknl = GPSMSG_LENGTH;
		printk(KERN_WARNING "command meet max length [%d] allowed,stripped!!\n",GPSMSG_LENGTH);
	}

	if (copy_from_user(messages, buff, lenknl))
	{
		return -EFAULT;
	}

	messages[GPSMSG_LENGTH] = '\0';

	if (0 == strncmp(messages, "off", 3))                                  //gps off branch
	{
		memset(sirf_status,0,sizeof(sirf_status));
		strncpy(sirf_status, "off",3);
		gps_power_off();
	} 
	else if (0 == strncmp(messages, "on", 2))                        //gps on branch
	{
		memset(sirf_status,0,sizeof(sirf_status));
		strncpy(sirf_status, "on",2);
		gps_power_on();
	} 
	else if (0 == strncmp(messages, "reset", 5))                    //gps reset branch
	{
		memset(sirf_status,0,sizeof(sirf_status));
		strncpy(sirf_status, messages,strlen(messages));
       ...
       else if (0 == strncmp(messages, "sirfon", 6))                   //gps sirfon branch
	{
		memset(sirf_status,0,sizeof(sirf_status));
		strncpy(sirf_status, messages,strlen(messages));
		ret = sscanf(messages, "%s %d", buffer, &flag);
	...
	{
		printk(KERN_NOTICE "usage: echo {on/off} > /proc/driver/sirf\n");
	}
...
static struct persistent_ram persistent_trace_ram = {
	.descs = &desc,
	.num_descs = 1,
};

static struct resource persistent_trace_resources[] = {
	{
		.flags = IORESOURCE_MEM,
	},
};
...
static struct platform_device persistent_trace_device = {
	.name		= "persistent_trace",
	.id		= -1,
	.num_resources	= ARRAY_SIZE(persistent_trace_resources),
	.resource	= persistent_trace_resources,
};

static void __init tegra_trace_debug_init(void)
{
	int err;

	err = platform_device_register(&persistent_trace_device);
	if (err)
		pr_err("%s: ram console registration failed (%d)!\n",
			__func__, err);
}

static void __init tegra_trace_debug_reserve(unsigned long persistent_trace_size)
{
	int ret;

	persistent_trace_ram.start = memblock_end_of_DRAM() - persistent_trace_size;
	persistent_trace_ram.size = persistent_trace_size;
	persistent_trace_ram.descs->size = persistent_trace_size;

	INIT_LIST_HEAD(&persistent_trace_ram.node);

	ret = persistent_ram_early_init(&persistent_trace_ram);
	if (ret)
		goto fail;
...
#elif defined(CONFIG_RAM_TRACER)

static struct resource ram_trace_resources[] = {
	{
		.flags = IORESOURCE_MEM,
	},
};
static struct platform_device ram_trace_device = {
	.name 		= "ram_trace",
	.id 		= -1,
	.num_resources	= ARRAY_SIZE(ram_trace_resources),
	.resource	= ram_trace_resources,
};

static void __init tegra_trace_debug_reserve(unsigned long ram_trace_size)
{
	struct resource *res;
	long ret;

	res = platform_get_resource(&ram_trace_device, IORESOURCE_MEM, 0);
	if (!res)
		goto fail;
	res->start = memblock_end_of_DRAM() - ram_trace_size;
	res->end = res->start + ram_trace_size - 1;
	ret = memblock_remove(res->start, ram_trace_size);
	if (ret)
		goto fail;
...
fail:
	ram_trace_device.resource = NULL;
	ram_trace_device.num_resources = 0;
	pr_err("Failed to reserve memory block for ram logger\n");
}
...
static void __init tegra_trace_debug_init(void)
{
	...
	err = platform_device_register(&ram_trace_device);
	if (err) {
		pr_err("%s: ram_trace registration failed (%d)!\n", __func__, err);
	}
}
...
static void __init tegra_pluto_init(void)
{
	...
	pluto_uart_init();
	pluto_audio_init();
	//ZTE: add for audio codec by wangtao +++
#ifdef CONFIG_SND_SOC_TEGRA_MAX9809X
	zte_audio_init();	
#endif
	//ZTE: add for audio codec by wangtao ---
	platform_add_devices(pluto_devices, ARRAY_SIZE(pluto_devices));
	tegra_ram_console_debug_init();
	tegra_trace_debug_init();
	tegra_io_dpd_init();
	pluto_sdhci_init();
...
static void __init tegra_pluto_reserve(void)
{
	...
	tegra_trace_debug_reserve(SZ_64K);
}
