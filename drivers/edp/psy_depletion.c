 ...
 *  >4.05 400
 * */
static int mdm_low_threshhold = 4000000; /*uV*/
module_param(mdm_low_threshhold, int, 0644);

static int mdm_high_threshhold = 4050000; /*uV*/
module_param(mdm_high_threshhold, int, 0644);
...
static unsigned int depl_calc(struct depl_driver *drv)
{
	...
	s64 pbat_gain;
	s64 depl;
	static unsigned char once_only = 1;

	ocv = depl_psy_ocv(drv);
	rbat = depl_rbat(drv);

	/* Dynamic adjust the Vsysmin, 3.4V OCV < 3.75, 3.5 OCV > 3.75*/
	#if 0
	if ( ocv >= 3900000)
	{
	    drv->pdata->vsys_min = 3200000;
	}
	else if ( (ocv > 3750000) && (ocv < 3900000) )
	{
	    drv->pdata->vsys_min = 3450000;
	}
	else
	{
	    drv->pdata->vsys_min = 3400000;
	}
	#endif

	if( (3 == zte_get_board_id()) && (1 == once_only))
	{
	    drv->pdata->vsys_min = 3300000;
	    once_only = 2;
	}
...
#if 0
	if (!drv->mdm_edp) {
		drv->mdm_edp = edp_get_client("lc171x_modem");
	}
	if (!drv->mdm_edp)
		printk("dpu:fail to get lc171x_modem edp client\n");
	else {
		struct edp_client *edp = drv->mdm_edp;
		int ocv = depl_psy_ocv(drv);

	 if (ocv > mdm_high_threshhold)
			edp_update_client_request(edp, edp->num_states - 2, NULL);
	else if (ocv < mdm_low_threshhold)
			edp_update_client_request(edp, 0, NULL);	 
	}
#endif	
}
