...
struct palmas_gpio *g_gpio;//add by caixiaoguang 20120227
...
/*ZTE:add by caixiaoguang for led ctrl 20120227++*/
void palmas_redlight_ctrl(int on_off)
{
    if(1==on_off)
        {
            palmas_gpio_set(&g_gpio->gpio_chip,1,0);//low level,led on
        }
    else
        {
            palmas_gpio_set(&g_gpio->gpio_chip,1,1);//high level,led off
        }    
}
void palmas_greenlight_ctrl(int on_off)
{  
    if(1==on_off)
        {
            palmas_gpio_set(&g_gpio->gpio_chip,2,0);//low level,led on
        }
    else
        {
            palmas_gpio_set(&g_gpio->gpio_chip,2,1);//high level,led off
        }
}
/*ZTE:add by caixiaoguang for led ctrl 20120227--*/
...
static int __devinit palmas_gpio_probe(struct platform_device *pdev)
{
    ...
    /*ZTE:add by caixiaoguang 20120227 for GPIO init++*/
    palmas_gpio_set(&gpio->gpio_chip,1,1);    
    palmas_gpio_set(&gpio->gpio_chip,2,1);
    palmas_gpio_direction_out(&gpio->gpio_chip,1,0);
    palmas_gpio_direction_out(&gpio->gpio_chip,2,0);
    g_gpio = gpio;
    /*ZTE:add by caixiaoguang 20120227 for GPIO init--*/    
	return ret;
