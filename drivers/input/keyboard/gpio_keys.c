...
static struct wake_lock pkw_wakelock;
...
static void gpio_keys_gpio_report_wake(struct gpio_button_data *bdata)
{
	...
	dev_info(&input->dev, "%s(%d) report wake event(pressed)\n", desc,	button->code);
...
static void gpio_keys_gpio_report_event(struct gpio_button_data *bdata)
{
	...
	if(state == bdata->key_pressed) return;
					 ...
					 const struct gpio_keys_button *button)
{
	...
	if (gpio_is_valid(button->gpio)) {
		...
		bdata->key_pressed = (gpio_get_value_cansleep(button->gpio) ? 1 : 0) ^ button->active_low;
	...
	if(bdata->irq == 737){
		...
		printk(KERN_INFO "register keypower irq 737.\n");
	}
...
static int __devinit gpio_keys_probe(struct platform_device *pdev)
{
	...
	wake_lock_init(&pkw_wakelock, WAKE_LOCK_SUSPEND, "PowerKeyResume");
	return 0;
...
static int __devexit gpio_keys_remove(struct platform_device *pdev)
{
	...
	wake_lock_destroy(&pkw_wakelock);
