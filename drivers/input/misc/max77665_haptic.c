...
/*ZTE:add by caixiaoguang 20130319 for vibrator++*/
#include "../../staging/android/timed_output.h"
static int timeout;
extern void msleep(unsigned int msecs);
struct max77665_haptic *g_chip;
#define VIBRETOR_ENABLE_GPIO	    TEGRA_GPIO_PH3
/*ZTE:add by caixiaoguang 20130319 for vibrator--*/
...
static void max77665_haptic_enable(struct max77665_haptic *chip, bool enable)
{
	...
	chip->enabled = enable;
    printk("[%s]CXG get vibrate enable=%d\n",__func__,enable);
	if (enable) {
		regulator_enable(chip->regulator);
		max77665_haptic_configure(chip);
		if (chip->mode == MAX77665_EXTERNAL_MODE)
			//pwm_enable(chip->pwm);
                         gpio_set_value(VIBRETOR_ENABLE_GPIO, 1);
	} else {
		//max77665_haptic_configure(chip);
		max77665_write_reg(chip, MAX77665_HAPTIC_REG_CONF2, 0x02);
        max77665_write(chip->dev->parent, MAX77665_I2C_SLAVE_PMIC,
				MAX77665_PMIC_REG_LSCNFG, 0x2A);
            
		if (chip->mode == MAX77665_EXTERNAL_MODE)
			//pwm_disable(chip->pwm);
			gpio_set_value(VIBRETOR_ENABLE_GPIO, 0);
...
static void vibrator_enable(struct timed_output_dev *dev, int value)
{
    ...
    timeout = value;    
 	if (!g_chip)
		return;
    //printk("[%s]CXG get vibrate timeout=%d\n",__func__,timeout);
	if (timeout) {
     #if 0
        ret = max77665_haptic_set_duty_cycle(g_chip);
		if (ret) {
			dev_err(g_chip->dev, "set_pwm_cycle failed\n");
			return;
		}
     #endif   
		max77665_haptic_enable(g_chip, true);
		msleep(timeout);		
		max77665_haptic_enable(g_chip, false);
	} else {
		max77665_haptic_enable(g_chip, false);
	}
}
...
static int __devinit max77665_haptic_probe(struct platform_device *pdev)
{
    ...
    status = timed_output_dev_register(&vibrator_dev);//ZTE:add by caixiaoguang 20130319 for vibrator
    if (status)
        {
            dev_err(&pdev->dev, "unable to register output dev\n");
            return -EINVAL;
        }
	...
	if (chip->mode == MAX77665_EXTERNAL_MODE) {
        ...
        ret = gpio_request(VIBRETOR_ENABLE_GPIO, "vibrator_en");
		if (ret < 0)
			pr_err("%s: gpio_request failed for gpio %s\n",
				__func__, "VIBRETOR_ENABLE_GPIO");
        gpio_direction_output(VIBRETOR_ENABLE_GPIO, 0);
	}
...
static int __devexit max77665_haptic_remove(struct platform_device *pdev)
{
	...
	destroy_work_on_stack(&chip->work);
    timed_output_dev_unregister(&vibrator_dev);//ZTE:add by caixiaoguang 20130319 for vibrator--
