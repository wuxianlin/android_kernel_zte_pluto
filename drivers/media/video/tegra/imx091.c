...
static struct nvc_gpio_init imx091_gpios[] = {
	...
	{IMX091_GPIO_GP2, 0, "gp2", false, false},
};
...
static struct nvc_imager_cap imx091_dflt_cap = {
	...
	.focuser_guid		= FOCUSER_DW9714_GUID,//NVC_FOCUS_GUID(0),
...
{
	if (val) {
		imx091_gpio_wr(info, IMX091_GPIO_GP2, val);
...
static int imx091_vreg_init(struct imx091_info *info)
{
	...
	for (i = 0; i < ARRAY_SIZE(imx091_vregs); i++) {
		...
		if (IS_ERR(info->vreg[j].vreg)) {
			...
			printk("%s %s ERR: %d\n",__func__, info->vreg[j].vreg_name, (int)info->vreg[j].vreg);
...
static int imx091_pm_wr(struct imx091_info *info, int pwr)
{
	...
	printk("yangchunni test, pwr = %d, cfg = %d\n", pwr, info->pdata->cfg);
	...
	if (pwr == info->pwr_dev)
		return 0;
	printk("yangchunni test, update pwr = %d\n", pwr);
	switch (pwr) {
...
int tegra_camera_clk_onoff(int on)
{
	return 0;
}
			 ...
			 unsigned long arg)
{
	...
	case NVC_IOCTL_PARAM_WR:
		printk("\n imx091_ioctl: NVC_IOCTL_PARAM_WR!\n");
		err = imx091_param_wr(info, arg);
		return err;

	case NVC_IOCTL_PARAM_RD:
		printk("\n imx091_ioctl: NVC_IOCTL_PARAM_RD!\n");
		err = imx091_param_rd(info, arg);
		return err;

	case NVC_IOCTL_DYNAMIC_RD:
		printk("\n imx091_ioctl: NVC_IOCTL_DYNAMIC_RD!\n");
	...
	case NVC_IOCTL_MODE_WR:
		printk("\n imx091_ioctl: NVC_IOCTL_MODE_WR!\n");
	...
	case NVC_IOCTL_MODE_RD:
		 ...
		 */
		 printk("\n imx091_ioctl: NVC_IOCTL_MODE_RD!\n");
	...
	case NVC_IOCTL_CAPS_RD:
		printk("\n imx091_ioctl: NVC_IOCTL_CAPS_RD!\n");
	...
	case NVC_IOCTL_STATIC_RD:
		printk("\n imx091_ioctl: NVC_IOCTL_STATIC_RD!\n");
	...
	const struct i2c_device_id *id)
{
	...
	printk("%s info->pdata->cfg= %d\n", __func__, info->pdata->cfg);
