...
imx132_write_reg(struct i2c_client *client, u16 addr, u8 val)
{
	...
	dev_err(&client->dev, "%s:i2c write failed, 0x%x = 0x%x\n",
			__func__, addr, val);
...
imx132_set_mode(struct imx132_info *info, struct imx132_mode *mode)
{
 ...
 #if 0
 for(i = 0;i < 100; i++)err = imx132_write_reg(info->i2c_client,0x310A,0xaa);
 #endif 
