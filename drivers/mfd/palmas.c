...
static struct extcon_dev palmas_edev;
	...
	for (i = 0; i < d->num_mask_regs; i++) {
		...
		if (ret < 0)
			dev_err(d->palmas->dev, "Failed to sync masks in %x\n",
					d->irq_regs->mask_reg[i].reg_add);
		#ifdef CONFIG_ZTE_PMU_RTC_DRV_DEBUG
		/* Take dump if RTC ALARM is getting set */
		if (i == 1) {
			if (d->mask_value[i] & 0x1)
				WARN_ON(1);
		}
		#endif
	}
...
static irqreturn_t palmas_irq_thread(int irq, void *data)
{
	...
	for (i = 0; i < d->num_mask_regs; i++) {
		ret = palmas_read(d->palmas,
				...
				&d->status_value[i]);
#ifdef CONFIG_ZTE_PMU_RTC_DRV_DEBUG
		pr_info("%s(): i=%d, base: 0x%04x reg: 0x%04x status: 0x%04x, mask: 0x%04x\n", __func__, 
            i, d->irq_regs->status_reg[i].reg_base, d->irq_regs->status_reg[i].reg_add,
...
#ifdef CONFIG_ZTE_PMU_RTC_DRV_DEBUG
    {
		...
		if (ret != 0)
		    dev_err(d->palmas->dev,
        		    "Failed to get line register: %d\n", ret);
		pr_info("%s():  base: 0x210 reg: 0x%04x  line register : 0x%04x \n", __func__,
                             index_count, status_value[i]);
		index_count += 5;
          }
    }
#endif
...
/*ZTE:add by caixiaoguang 20130123 for tps65914++*/
extern void max77665_start_charging(void);
extern void max77665_stop_charging(void);
void battery_update_by_interrupt(void);
struct delayed_work		vbus_int_work;
extern void msleep(unsigned int msecs);
struct wake_lock charge_wakelock;
/*ZTE:add by caixiaoguang 20130123 for tps65914++*/
...
/*ZTE:add by caixiaoguang 20130123 for tps65914 interrupt++*/
int palmas_vbus_status(void)
{
    unsigned int reg, addr;
	int slave;
    int vbus_present = 0;
    struct palmas *palmas = palmas_dev;
    slave = PALMAS_BASE_TO_SLAVE(PALMAS_INTERRUPT_BASE);
	addr = PALMAS_BASE_TO_REG(PALMAS_INTERRUPT_BASE, 0x0C);
...
static irqreturn_t palmas_vbus_interrupt(int irq, void *data)
{        
    schedule_delayed_work(&vbus_int_work, 100);
    /*battery_update_by_interrupt();*/
    return IRQ_HANDLED;
}

#ifdef CONFIG_ZTE_PMU_RTC_DRV_DEBUG
void palmas_irq_reg_dump()
{
	struct palmas_irq_chip_data *d = palmas_dev->irq_chip_data;
	int ret, i;
	unsigned int	mask_value[PALMAS_MAX_INTERRUPT_MASK_REG];
	unsigned int	status_value[PALMAS_MAX_INTERRUPT_MASK_REG];
	...
	for (i = 0; i < d->num_mask_regs; i++) {
		...
		if (ret != 0)
			dev_err(palmas_dev->dev,
				"Failed to get status register: %d\n", ret);
		ret = palmas_read(d->palmas,
				d->irq_regs->mask_reg[i].reg_base,
				d->irq_regs->mask_reg[i].reg_add,
				&mask_value[i]);
		if (ret != 0)
			dev_err(palmas_dev->dev,
				"Failed to get mask register: %d\n", ret);

		pr_info("%s(): i=%d, base: 0x%04x reg: 0x%04x  status: 0x%04x, mask: 0x%04x\n", __func__,
                        i, d->irq_regs->status_reg[i].reg_base, d->irq_regs->status_reg[i].reg_add, status_value[i], mask_value[i]);
	}
}
EXPORT_SYMBOL(palmas_irq_reg_dump);
#endif

void palmas_irq_reg_dump_all(void)
{
	struct palmas_irq_chip_data *d = palmas_dev->irq_chip_data;
	int ret, i;
	unsigned int	status_value[21]={0};
	...
	for (i = 0; i < 21; i++) {
		...
		pr_info("%s():  base: 0x210 reg: 0x%04x  status: 0x%04x \n", __func__,
                         i, status_value[i]);
	}
       ret = palmas_read(d->palmas,
				0x1A0,
        ...
        pr_info("%s():  base: 0x1A0 reg: 0x000a  status: 0x%04x \n", __func__, osc_value);
}
EXPORT_SYMBOL(palmas_irq_reg_dump_all);
...
void palmas_vbus_det_fun(struct work_struct *work)
{    
    int vbus_present = 0;
    vbus_present = palmas_vbus_status();
    if(vbus_present)
        {
            printk("[%s]CXG det usb insert!\n",__func__);	
 			extcon_set_cable_state(&palmas_edev, "USB", true);
            if( !wake_lock_active(&charge_wakelock) )
		        wake_lock(&charge_wakelock);
            /*max77665_start_charging();*/
        }
    else
        {
            printk("[%s]CXG det usb pull out!\n",__func__);
			extcon_set_cable_state(&palmas_edev, "USB", false);
            if( wake_lock_active(&charge_wakelock) )
		        wake_unlock(&charge_wakelock);
            /*max77665_stop_charging();*/
        }
			    ...
			    const struct i2c_device_id *id)
{
	...
	palmas_edev.name = "PALMAS_USB";
	palmas_edev.supported_cable = palmas_extcon_cable;
	ret = extcon_dev_register(&palmas_edev, palmas->dev);
	if (ret < 0) {
		dev_err(palmas->dev, "extcon device register failed %d\n", ret);
		return ret;
	}
#if 0
/*ZTE:add by caixiaoguang 20130123 for tps65914 interrupt++*/
    wake_lock_init(&charge_wakelock, WAKE_LOCK_SUSPEND, "Charge");
    INIT_DELAYED_WORK_DEFERRABLE(&vbus_int_work, palmas_vbus_det_fun);
    ret = request_threaded_irq(764, NULL,
		palmas_vbus_interrupt, IRQF_ONESHOT,
		"palmas-vbus", palmas);
	if (ret < 0) {
		dev_err(palmas->dev, "IRQ is not free.\n");
		return ret;
	} 
/*ZTE:add by caixiaoguang 20130123 for tps65914 interrupt--*/
#endif
	...
	palmas_dev = palmas;
    /* schedule_delayed_work(&vbus_int_work, 0);*/
	return ret;
...
static int palmas_i2c_remove(struct i2c_client *i2c)
{
	...
	palmas_del_irq_chip(palmas->irq, palmas->irq_chip_data);
        wake_lock_destroy(&charge_wakelock);
	return 0;
}
