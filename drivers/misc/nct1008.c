...
static void nct1008_get_temp_zte(struct nct1008_data *data)
{
	s16 temp1 = 0;
	s16 temp = 0;
	u8 temp2 = 0;
	...
	value = nct1008_read_reg(data->client, LOCAL_TEMP_RD);
	if (value < 0)
		goto error;
	temp1 = value_to_temperature(data->plat_data.ext_range, value);

	value = nct1008_read_reg(data->client, EXT_TEMP_RD_LO);
	if (value < 0)
		goto error;
	temp2 = (value >> 6);
	value = nct1008_read_reg(data->client, EXT_TEMP_RD_HI);
	if (value < 0)
		goto error;
	temp = value_to_temperature(data->plat_data.ext_range, value);

	printk("nct1008_get_temp_zte local=%d ext=%d.%d\n",temp1, temp, temp2 * 25);
      return;
error:
	printk("nct1008_get_temp_zte error\n");
      return;
}
...
static int nct1008_configure_sensor(struct nct1008_data *data)
{
	...
	err = nct1008_write_reg(client,
		LOCAL_TEMP_HI_LIMIT_WR, NCT1008_MAX_TEMP+EXTENDED_RANGE_OFFSET);//NCT1008_MAX_TEMP
	if (err)
		goto error;
	...
	//Hardware error make the ext temperature up to the max value
	...
	err = nct1008_write_reg(client, OFFSET_WR, (ext_offset >> 8) & 0xFF);
	if (err < 0)
		goto error;
