...
long zte_get_skin_temp(void)
{
      return get_skin_temp;
}
...
#if 0
static ssize_t show_performance_sw(struct device *dev,
			struct device_attribute *da,
			char *buf)
{
	struct therm_estimator *est = dev_get_drvdata(dev);
...
static ssize_t set_performance_sw(struct device *dev,
			struct device_attribute *da,
			const char *buf, size_t count)
{
	struct therm_estimator *est = dev_get_drvdata(dev);
	...
	if (kstrtoint(buf, 0, &performance_sw))
		return -EINVAL;

	est->performance_sw= performance_sw;
