...
#define SDHOST_3V3_OCR_MASK	0x1
...
static irqreturn_t carddetect_irq(int irq, void *data)
{
	...
	tegra_host->card_present = (gpio_get_value(plat->cd_gpio) == CONFIG_MMC_PRESENT_PINLEVEL);
	printk("%s: card is %s\n", mmc_hostname(sdhost->mmc), \
						tegra_host->card_present?"plugged":"removed");
...
static int tegra_sdhci_resume(struct sdhci_host *sdhci)
{
	...
	if (gpio_is_valid(plat->cd_gpio)){
		tegra_host->card_present = (gpio_get_value(plat->cd_gpio) == CONFIG_MMC_PRESENT_PINLEVEL);
		pr_info("%s: sd card is %s while resuming\n", mmc_hostname(sdhci->mmc), \
						tegra_host->card_present?"plugged":"removed");
	}
...
static int __devinit sdhci_tegra_probe(struct platform_device *pdev)
{
	...
	if (gpio_is_valid(plat->cd_gpio)) {
		...
		tegra_host->card_present = (gpio_get_value(plat->cd_gpio) == CONFIG_MMC_PRESENT_PINLEVEL);
		printk("%s: card is %s", mmc_hostname(host->mmc), \
                        tegra_host->card_present?"present":"not present");
	...
	} else if (plat->mmc_data.ocr_mask & SDHOST_3V3_OCR_MASK) {
