...
/** ZTE_MODIFY: modified by djt 2012/08/07 for hotspot sleep policy, LAN setting and hidden ssid */
#define CMD_GET_ASSOC_STA_LIST	"GET_ASSOC_STA_LIST"
/** ZTE_MODIFY end djt 2012/08/07 */
...
int wifi_set_power(int on, unsigned long msec)
{
	...
	if (wifi_control_data && wifi_control_data->set_power) {
		...
		mutex_unlock(&edp_reg_mutex);
#endif
		if (msec && !on) // Add delay time before wifi chip poweroff, by yuanzongxuan
			msleep(msec);
