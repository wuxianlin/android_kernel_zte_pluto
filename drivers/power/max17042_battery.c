...
extern int max77665_charging_suspend(void);
extern int max77665_charging_resume(void);
extern int max77665_get_vbus_ovp_status(void);
...
/*ZTE:add by caixiaoguang 20130402 for get battery voltage++*/
int max17042_get_bat_voltage(void)
{
    int ret = 0;
    if (temp_client != NULL)
    {
        ret = max17042_read_reg(temp_client, MAX17042_VCELL);
	    if (ret < 0)
		    return ret;

	    ret = ret * 625 / 8;
    }     
    printk("[%s]CXG get bat_voltage=%d\n",__func__,ret);
    return ret;
}
int max17042_vcell_filter(void)
{
    int vol_local  = 0;
    int min_vol = 0;
    int iTemp[6] = {0};
    ...
    for(i=0;i<6;i++)
    {
        iTemp[i]= max17042_get_bat_voltage();
        msleep(100);
    }
...
int bat_cap_conversion(int cap)
{
	...
	{
		cap_disp=(cap_real*100)/95;
	}	
    #endif
	printk("[Max17042]  %s: cap_real(%d) and cap_disp(%d)...\n", __func__,cap_real, cap_disp);
	return cap_disp;
}
/*ZTE:  add by tengfei for optimization of battery capacity display, 2012.4.12, --*/
			    ...
			    union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		val->intval = ret >> 8;
		//chip->cap = val->intval;
    	/*ZTE:add by caixiaoguang for battery level 20130304 ++*/
        if((charge_full_flag==1) && (val->intval >= 95))
    	{
        		val->intval = 100;
    	}
		val->intval = bat_cap_conversion(val->intval);
        ...
        if(val->intval <= 2)
        {
            vell_local= max17042_vcell_filter();
            printk("[%s]CXG get low power level = %d, voltage = %d mv\n",__func__,val->intval,vell_local);
            if(vell_local > 3400000)
            {
                val->intval =1;
            }
            else
	...
	case POWER_SUPPLY_PROP_CURRENT_AVG:
		...
		break;
/*ZTE:delete by caixiaoguang 20130328 for charge status*/ 
#if 0
	case POWER_SUPPLY_PROP_STATUS:        
		if (chip->status)
        ...
        case POWER_SUPPLY_PROP_HEALTH: 
            temp_high_low = max17042_get_otp_status();
            if(2==temp_high_low)
                {
                    val->intval =  POWER_SUPPLY_HEALTH_OVERHEAT;
                }
            else if(1==temp_high_low)
                {
...
static void max17042_set_soc_threshold(struct max17042_chip *chip, u16 off)
{
	...
	soc_tr |= (soc - off);
#endif 
    soc_tr = 0xFF01;//MAX disable,MIN 1%
...
static irqreturn_t max17042_thread_handler(int id, void *dev)
{
	...
	val = max17042_read_reg(chip->client, MAX17042_STATUS);
    printk("[%s]CXG enter low soc alrt and get reg =0x%x\n",__func__,( val & 0x0400 ));
...
/*ZTE:add by caixiaoguang 20130301 for charge status ++*/
int max17042_get_otp_status(void)
{
    int ret = 0;
    int temp_high_low = 0;
    if (temp_client != NULL)
    {
        ret = max17042_read_reg(temp_client, MAX17042_STATUS); 
    } 
    ret &= 0x2200;
    if(0x2000==ret)
        {
            temp_high_low=2;//overheat
            printk("[%s]CXG get temp_high_low=%d,so batter over heat!\n",__func__,temp_high_low);
        }
    else if(0x0200==ret)
        {
            temp_high_low=1;//cold
            printk("[%s]CXG get temp_high_low=%d,so batter too cold!\n",__func__,temp_high_low);
        }
    else
        {
            temp_high_low=0;//good
        } 
    return temp_high_low;
}
void max17042_get_status(struct max17042_chip *chip)
{    
    int ovp_flag = 0;
    int otp_flag = 0;
    if (chip->pdata->charger_online())
    {
        ...
        {   
            ovp_flag = max77665_get_vbus_ovp_status();
            otp_flag = max17042_get_otp_status();
            if((ovp_flag == 0)&&(otp_flag==0))
                {
                ...
                {
                    chip->charge_status = POWER_SUPPLY_STATUS_NOT_CHARGING;
                    max77665_charging_suspend();
                } 
            if(chip->soc==100)
            charge_full_flag=1;
        }	
...
void battery_update_by_interrupt(void)
{
    ...
    {
		schedule_work(&tmp_chip->charge_indicate_work); 
    }   
			...
			const struct i2c_device_id *id)
{
    ...
    printk("[%s]CXG get MAX17042_STATUS reg =0x%x\n",__func__,reg);    
	if (reg & STATUS_POR_BIT) {
		if (chip->pdata->enable_por_init && chip->pdata->config_data) {
			ret = max17042_init_chip(chip);
			if (ret)
				return ret;
            chip->init_complete = 1;//ZTE:add by caixiaoguang 20130123 for max17042
		}
	} else {
		chip->init_complete = 1;
	}
