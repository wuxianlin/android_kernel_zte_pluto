...
static struct palmas *p_palmas = NULL;
...
static int palmas_rtc_alarm_irq_enable(struct device *dev, unsigned enabled)
{
	...
	dev_err(dev, "%s(): enabled %u\n", __func__, enabled);

	palmas_rtc_read(palmas,PALMAS_RTC_INTERRUPTS_REG,&val);
	if (enabled)
	...
	palmas_rtc_write(palmas,
		PALMAS_RTC_INTERRUPTS_REG, val);
      palmas_irq_reg_dump();
      palmas_rtc_read(palmas,PALMAS_RTC_INTERRUPTS_REG,&val_read);
      if(val != val_read)
     {
             printk("palmas_rtc_alarm_irq_enable error");
     }
      dev_info(dev, "%s():  val =0x%x val_read=0x%x\n", __func__, val,val_read);
      return 0;
#else
...
static ssize_t rtc_proc_read(struct file *filp,char *buff, size_t len, loff_t *off)
{
    int i = 0;
    uint8_t reg_val[21];
    printk("++++++++++++rtc_proc_read++++++++++++\n");
    palmas_rtc_bulk_read(p_palmas, 0x8, &reg_val[0],6);

    for(i = 0;i < 6;i++)
    {
        printk("pmu rtc reg address: 0x%x,value:0x%x\n",(0x8+i),reg_val[i]);
    }
    palmas_rtc_bulk_read(p_palmas, 0x10, &reg_val[11], 7);
    for(i = 0;i < 6;i++)
    {
...
/*ZTE:add by caixiaoguang for set a flag before power off--*/
/*ZTE:add by caixiaoguang for set usb current high when needed++*/
extern void max77665_set_usb_high_current_flag(int high);
static int set_usb_high_current_proc_write(struct file *file, const char __user *buffer,
			   unsigned long count, void *data)
{
	...
	if (copy_from_user(buf, buffer, count))
		return -EFAULT;
	sscanf(buf, "%c", &cmd);
    printk("[%s]CXG set_usb_high_current=%s,cmd = %c\n ",__func__,buf,cmd);
    if('1' == cmd )
        {
            max77665_set_usb_high_current_flag(1);
        }
    else if('0' == cmd )
        {
            max77665_set_usb_high_current_flag(0);
        }
    return 1;
}

static struct file_operations set_usb_high_current_proc_ops = {

   	.owner   = THIS_MODULE, 
   	.write = set_usb_high_current_proc_write,
};

static int create_set_usb_high_current_proc_file(void)	
{  
	struct proc_dir_entry *entry;        
       entry = create_proc_entry("driver/set_high_current", 0666, NULL);     
   	if (entry)  
       	entry->proc_fops = &set_usb_high_current_proc_ops;           
       return 0;
}
/*ZTE:add by caixiaoguang for set usb current high when needed--*/
static int __devinit palmas_rtc_probe(struct platform_device *pdev)
{
	...
	palmas_rtc_read_time(&pdev->dev, &tm);
	if ((tm.tm_year == RTC_YEAR_OFFSET + RTC_POR_YEAR) &&
		(tm.tm_mon == (RTC_POR_MONTH - 1)) &&
		(tm.tm_mday == RTC_POR_DAY)) {
		if (rtc_pdata->time.tm_year < 2000 ||
			rtc_pdata->time.tm_year > 2100) {
			dev_err(&pdev->dev, "Invalid platform data\n");
			memset(&rtc_pdata->time, 0, sizeof(rtc_pdata->time));
			rtc_pdata->time.tm_year = 2011;
			rtc_pdata->time.tm_mday = 1;
		}
		palmas_rtc_set_time(&pdev->dev, &rtc_pdata->time);
	}
	#endif
	p_palmas = palmas;
	create_rtc_proc_file();
    create_set_power_off_flag_proc_file();
    create_set_usb_high_current_proc_file();
	return 0;
}
