...
struct wake_lock alarm_rtc_wake_lock;

static int is_wakeup(enum android_alarm_type type)
{
	if (type == ANDROID_ALARM_RTC_WAKEUP ||
			type == ANDROID_ALARM_ELAPSED_REALTIME_WAKEUP ||
			type == ANDROID_ALARM_POWEROFF_WAKEUP)
		return 1;
	return 0;
}
...
static long alarm_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	int ret;
	struct rtc_device *alarm_rtc_dev = alarmtimer_get_rtcdev();
	...
	case ANDROID_ALARM_CLEAR(0):
		...
		if (alarm_type == ANDROID_ALARM_POWEROFF_WAKEUP)
		{
			...
			printk(KERN_ERR"ANDROID_ALARM_CLEAR ANDROID_ALARM_POWEROFF_WAKEUP\n");
		}
...
from_old_alarm_set:
		...
		{
		  printk("alarm_rtc_dev=NULL\n");
		}
		else
		{
		  printk("alarm_rtc_dev exists,alarm_type = 0x%x\n",alarm_type);
		}
		if (alarm_type == ANDROID_ALARM_POWEROFF_WAKEUP)
		{
		    if(alarm_rtc_dev !=NULL)
			...
			set_normalized_timespec(&rtc_delta,
					wall_time.tv_sec - rtc_current_timespec.tv_sec,
					wall_time.tv_nsec);
			...
			//modify lupeng for the system time late 20120801
			rtc_alarm_time = rtc_alarm_time + 2;

			rtc_time_to_tm(rtc_alarm_time, &rtc_alarm.time);
			rtc_new_rtc_time = rtc_alarm.time;
			printk("alrm->tm_hour=%d alrm->tm_min=%d alrm->tm_sec=%d\n",
			...
			{
                        ret = rtc_set_alarm(alarm_rtc_dev, &rtc_alarm);
                        printk("rtc_set_alarm error try again ret=%d \n",ret);
			}
...
static void devalarm_triggered(struct devalarm *alarm)
{
	...
	printk("devalarm_triggered type %d\n", alarm->type);
...
void alarm_triggered_func(void *p)
{
	unsigned long flags;
	...
	rtc->irq_data &= ~RTC_AF;
	wake_lock_timeout(&alarm_wake_lock, 3 * HZ);
	alarm_type_mask = 1U << ANDROID_ALARM_POWEROFF_WAKEUP;
	spin_lock_irqsave(&alarm_slock, flags);
	if (alarm_enabled & alarm_type_mask) {
		pr_alarm(INT, "alarm_triggered_func: type = poweroff wakeup\n");
		alarm_enabled &= ~alarm_type_mask;
		alarm_pending |= alarm_type_mask;
		wake_up(&alarm_wait_queue);
	}
...
static int __init alarm_dev_init(void)
{
	...
	alarm_init(&alarms[ANDROID_ALARM_POWEROFF_WAKEUP].u.alrm,
			ALARM_BOOTTIME, devalarm_alarmhandler);
