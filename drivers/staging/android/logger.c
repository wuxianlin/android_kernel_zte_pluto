...
static struct work_struct kernel_log_wq;
...
static void logger_kernel_write(struct work_struct *work)
{
...
logger_console_write(struct console *console, const char *s, unsigned int count)
{
	...
	if (unlikely(!keventd_up()))
		return;
	schedule_work_on(0, &kernel_log_wq);
}
...
static int __init logger_console_init(void)
{
	...
	INIT_WORK(&kernel_log_wq, logger_kernel_write);
