...
struct tegra_uart_port {
	...
	bool			uart_suspend;
	struct oem_modem_edp 	*modem_edp;
};
...
static void tegra_start_next_tx(struct tegra_uart_port *t)
{
	...
	if (t->uart_suspend) {
		dev_err(t->uport.dev, "Could not start next tx in suspend state!\n");
		return;
	}
...
static void tegra_start_tx(struct uart_port *u)
{
	...
	if (t->uart_suspend) {
		dev_err(t->uport.dev, "Could not start tx in suspend state!\n");
		return;
	}
	xmit = &u->state->xmit;
...
static irqreturn_t tegra_uart_isr(int irq, void *data)
{
	...
	t  = container_of(u, struct tegra_uart_port, uport);
	if (t->uart_suspend) {
		dev_err(t->uport.dev, "%s: unexcepted irq in suspend state!\n", __func__);
	}
	while (1) {
		...
		if (iir & UART_IIR_NO_INT) {
			...
			spin_unlock_irqrestore(&u->lock, flags);
			if (t->uart_suspend) {
				dev_err(t->uport.dev, "%s: exit in suspend state!\n", __func__);
			}
			return IRQ_HANDLED;
		}
...
extern ssize_t oem_modem_estate_show(struct oem_modem_edp *modem_edp, struct device_attribute *attr, char *buf);
extern ssize_t oem_modem_estate_store(struct oem_modem_edp *modem_edp, struct device_attribute *attr, const char *buf, size_t size);
extern int oem_modem_edp_update_client_request_sync(struct oem_modem_edp *modem_edp, int estat);
extern int oem_modem_edp_update_client_request_async(struct oem_modem_edp *modem_edp, int estate);
extern int oem_modem_edp_init(struct oem_modem_edp *modem_edp);
extern void oem_modem_edp_remove(struct oem_modem_edp *modem_edp);

static ssize_t lc171x_modem_estate_show(struct device *pdev, struct device_attribute *attr,
                        char *buf)
{
	struct tegra_uart_port *t = platform_get_drvdata(pdev);
        struct oem_modem_edp *modem_edp = t->modem_edp;

        return oem_modem_estate_show(modem_edp, attr, buf);
}

static ssize_t lc171x_modem_estate_store(struct device *pdev, struct device_attribute *attr,
                        const char *buf, size_t size)
{
        struct tegra_uart_port *t = platform_get_drvdata(pdev);
        struct oem_modem_edp *modem_edp = t->modem_edp;

        return oem_modem_estate_store(modem_edp, attr, buf, size);
}

static DEVICE_ATTR(modem_estate, S_IRUSR | S_IWUSR,
                lc171x_modem_estate_show,
                lc171x_modem_estate_store);
#endif
...
{
#ifdef CONFIG_LC171X_MODEM_EDP
        struct oem_modem_edp_data *modem_edp_pdata;
        struct oem_modem_edp *modem_edp;
#endif
...
#ifdef CONFIG_LC171X_MODEM_EDP
	if ( pdata->is_modem ) {
        	modem_edp_pdata = pdata->modem_edp_pdata;
        	if (!modem_edp_pdata) {
                	pr_err("%s(%d): missing modem edp platform data!\n", __func__, __LINE__);
                	return -EINVAL;
        	}
	}
#endif
...
#ifdef CONFIG_LC171X_MODEM_EDP
	if ( pdata->is_modem ) {
        	modem_edp = kzalloc(sizeof(struct oem_modem_edp), GFP_KERNEL);
        	if (!modem_edp) {
                	pr_err("%s(%d): memoray allocate for modem_edp failed!\n", __func__, __LINE__);
                	ret = PTR_ERR(modem_edp);
                	return ret;
        	}

        	modem_edp->pdev = &pdev->dev;
        	strncpy(modem_edp->edp_mgr_name, modem_edp_pdata->edp_mgr_name, EDP_NAME_LEN);
        	modem_edp->edp_mgr_name[EDP_NAME_LEN-1] = '\0';
        	modem_edp->edp_client = modem_edp_pdata->edp_client;

        	ret = oem_modem_edp_init(modem_edp);
        	if (ret) {
                	pr_err("%s(%d): init lc171x modem edp failed, errno(%d)!\n",
                       		 __func__, __LINE__, ret);
                	kfree(modem_edp);
                	return ret;
        	}

        	ret = device_create_file(modem_edp->pdev, &dev_attr_modem_estate);
        	if (ret) {
                	pr_err("%s(%d): can't create attribute file node, errno(%d)!\n",
                        	__func__, __LINE__, ret);
                	oem_modem_edp_remove(modem_edp);
                	kfree(modem_edp);
                	return ret;
        	}
        	//dev_set_drvdata(modem_edp->pdev, modem_edp);
		t->modem_edp = modem_edp;
	}
#endif
...
static int __devexit tegra_uart_remove(struct platform_device *pdev)
{
	...
	if ( pdata->is_modem ) {
	        ...
	        if (!modem_edp) {
        	        pr_err("%s(%d): failed to get modem_edp!\n", __func__, __LINE__);
                	return -EINVAL;
        	}
        	device_remove_file(modem_edp->pdev, &dev_attr_modem_estate);
        	oem_modem_edp_remove(modem_edp);
        	kfree(modem_edp);
        	modem_edp = NULL;
	}
...
static int tegra_uart_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (pdata && pdata->sleep_ctrl)
		pdata->sleep_ctrl(u, 1, 0);
	t->uart_suspend = true;
	uart_suspend_port(&tegra_uart_driver, u);
	if (pdata && pdata->sleep_ctrl)
		pdata->sleep_ctrl(u, 1, 1);
