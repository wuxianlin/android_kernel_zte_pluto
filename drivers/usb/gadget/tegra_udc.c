...
int fsl_charger_detect(void)
{
	...
	if (!udc->vbus_active) {
		pr_debug("%s(%d): VBUS inactive!\n",  __func__, __LINE__);
		return ret;
	}
	
	ret = udc->connect_type;
...
static void dr_controller_run(struct tegra_udc *udc)
{
	...
	/* this will increase throughput by a factor of 4 to 5 for adb */
...
/*++ ZTE: cuijian add for usb switch 20130710 ++*/
void tegra_udc_disconnect(void){
	printk("%s \n", __func__);
	if (the_udc)
		tegra_vbus_session(&the_udc->gadget, 0);
}



void tegra_udc_connect(void){
	printk("%s \n", __func__);
	if (the_udc)
		tegra_vbus_session(&the_udc->gadget, 1);
}
/*-- ZTE: cuijian add for usb switch 20130710 --*/
...
/*
	ZTE:cuijian disable udc wake up for permenent wakeup bug 20130609
