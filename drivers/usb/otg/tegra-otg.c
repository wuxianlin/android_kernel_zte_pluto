...
/* +++ZTE: added by liliguo for mhl USB_ID switch 20130118+++ */
#ifdef CONFIG_SII8240_MHL_TX
extern void CBUS_ID_Opened_AfterPlugedout( void );
#endif
/* ---ZTE: added by liliguo for mhl USB_ID switch 20130118--- */
...
static unsigned long enable_interrupt(struct tegra_otg_data *tegra, bool en)
{
	...
	/*++ ZTE:cuijian modify for detect a stable id signal 20130121*/
	//udelay(1);
	mdelay(10);
	val = otg_readl(tegra, USB_PHY_WAKEUP);
	/*-- ZTE:cuijian modify for detect a stable id signal 20130121*/
...
/*++ZTE:cj modify for check otg abnromal wakeup 20130503++*/
extern int otg_wake_record;
/*--ZTE:cj modify for check otg abnromal wakeup 20130503--*/
...
/*++ZTE:cj modify for check otg abnromal wakeup 20130503++*/
	if(otg_wake_record){
		otg_wake_record = 0;
		pr_info("tegra-otg:wakeup %s(%d) interrupt val = 0x%lx\n", __func__, __LINE__, val);
	}
/*--ZTE:cj modify for check otg abnromal wakeup 20130503--*/
...
/*++ ZTE:cj disable otg wakeup function 20130411 ++*/
#if 0
