		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (bd->ops) {
		if (brightness > bd->props.max_brightness){
			bd->props.brightness = bd->props.max_brightness - 1;
			backlight_update_status(bd);
			rc = count;
		}
		else {
...
static ssize_t backlight_store_maxbrightness(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
    unsigned long maxbrightness;
    int rc;
	struct backlight_device *bd = to_backlight_device(dev);
    printk("[BL] backlight_store_maxbrightness maxbrightness=%d\r\n",maxbrightness);
