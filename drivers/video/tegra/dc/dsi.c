						...
						struct tegra_dc_dsi_data *dsi)
{
	...
	val = DSI_PKT_LEN_6_7_LENGTH_6(hfp_pkt_len) | DSI_PKT_LEN_6_7_LENGTH_7(0);
	tegra_dsi_writel(dsi, val, DSI_PKT_LEN_6_7);
...
// int is_pad_need_re_cal = 1;      //zte modify :
...
//	if(is_pad_need_re_cal){
//		tegra_dsi_pad_calibration(dsi);
//		is_pad_need_re_cal = 0;
//		printk(KERN_ERR"kassey %s %d\n", __func__, __LINE__);
//	}
       tegra_dsi_pad_calibration(dsi);
...
{
	int err = 0;
/* ZTE_MODIFY added begin by liyibo 2013-09-05 */
	if (IS_ERR_OR_NULL(init_status)) {
		err = PTR_ERR(init_status);
		dev_err(&dc->ndev->dev, "tegra_dc_dsi_reset_lcd DSI host config failed\n");
		return err;
	}
/* ZTE_MODIFY added end  by   liyibo 2013-09-05 */
					...
					u32 n_cmd)
{
	...
	for (i = 0; i < n_cmd; i++) {
		...
		if (cur_cmd->cmd_type == TEGRA_DSI_GPIO_SET) {
			...
			//mdelay(1);/*ZTE: deleted by tong.weili to optimize LCD resume time 20130423*/
			if (err < 0)
				break;
		}
	}
...
//      is_pad_need_re_cal = 1; //zte modify :
...
void tegra_dc_dsi_reset_lcd(struct tegra_dc *dc)
{
    ...
    tegra_dsi_send_panel_cmd(dc, dsi,
                            dsi->info.dsi_reset_cmd,
                            dsi->info.n_reset_cmd);
